...
function applyTags(tags = [], children, noPTag = false, key) {
  ...
  for (const tag of tags) {
    ...
    if (tagName === 'a') {
      props.href = tag[1];
      props.className = 'text-base text-blue-500 transition-colors duration-200	hover:text-blue-300';
    }
    if (tagName === 'e') {
...
export function textBlock(text = [], noPTag = false, mainKey) {
  ...
  return React.createElement(noPTag ? React.Fragment : components.p, { key: mainKey }, ...children, noPTag);
}
