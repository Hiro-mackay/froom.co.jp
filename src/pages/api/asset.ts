...
export default async function notionApi(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!assetUrl || !blockId) {
      ...
      const { signedUrls = [], ...urlsResponse } = await getNotionAssetUrls(res, assetUrl, blockId);
